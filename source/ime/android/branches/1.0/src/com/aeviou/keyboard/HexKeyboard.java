	...
	public void onRelease(int x, int y) {
		...
		if (candidateBar.isOpen) {
			...
			if (startAtLetter == false) {
				...
				if (upperStatus) {
					InputLogic
							.HexKeySymbolPressed(pinyinContext, pinyin, false);
					setShiftStatus(false);

				} else if (hybridMode && pinyin != null
					...
					if (hybridMode) {
						if (pinyin != null
								&& ((pinyin.length() >= 2 && pinyin.charAt(0) == 'v') || (pinyin
										.length() == 1 && pinyin.charAt(0) == 'm'))) {
							if (pinyin.charAt(0) == 'v') {
								InputLogic.HexKeyLetterPressed(pinyinContext,
										String.valueOf(pinyin.charAt(1)));
							} else {
								InputLogic.HexKeyLetterPressed(pinyinContext,
										String.valueOf(pinyin.charAt(0)));
							}
							InputLogic.HexKeyPinyinPressed(pinyinContext,
									pinyin);
	...
	void fillMiddlePoint(int x, int y) {
		...
		if (sildeVector.isZero() == false) {
			...
			while (Math.abs(x - lastKey.centerX) > AeviouConstants.NORMAL_IME_HEXKEY_WIDTH
				...
				middleX = (int) ((lastKey.centerX + sildeVector.x
						* AeviouConstants.NORMAL_IME_HEXKEY_WIDTH));
				middleY = (int) ((lastKey.centerY + sildeVector.y
						* AeviouConstants.NORMAL_IME_HEXKEY_WIDTH));
