...
public class PinyinNode {
    ...
    int getChild(int current, char pinyin){
        ...
        while (i <= j){
            ...
            if (pinyin == value){
                ...
                lastAddress = PinyinUtils.getInteger(childrenBuffer, address + 2);
                lastFreqency = PinyinUtils.getInteger(childrenBuffer, address + 6);
    ...
    int getMaxFreqency(int current){
        if (lastCurrent == current){
            return lastFreqency;
        }
        try{
