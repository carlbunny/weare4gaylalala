...
//int             iTempInputWindowX, iTempInputWindowY;	//????¨º?¨¨?¨¬?¦Ì?¨¢¨´¨º¡À????¡ê?¨®?¨®¨²1a¡À¨º?¨²???¡ê¨º?
//
...
//MESSAGE_COLOR   inputWindowLineColor = { NULL, {0, 90 << 8, 160 << 8, 90 << 8} };	//¨º?¨¨??¨°?D¦Ì???¨¬?¨¦?
//XColor          colorArrow = { 0, 255 << 8, 0, 0 };	//?y¨ª¡¤¦Ì???¨¦?
//
//WINDOW_COLOR    inputWindowColor = { NULL, NULL, {0, 240 << 8, 255 << 8, 240 << 8} };
//MESSAGE_COLOR   cursorColor = { NULL, {0, 92 << 8, 210 << 8, 131 << 8} };

// *************************************************************
MESSAGE         messageUp[32];	//¨º?¨¨?¨¬?¨¦?2?¡¤???¨º?¦Ì??¨²¨¨Y
uint            uMessageUp = 0;

// *************************************************************
MESSAGE         messageDown[32];	//¨º?¨¨?¨¬???2?¡¤???¨º?¦Ì??¨²¨¨Y
uint            uMessageDown = 0;
...
//    for (i = 0; i < uMessageDown; i++) {
//	//?¨¨¨®?iInputWindowDownWidth¡Á¡Â?a¨°???¨¢¨´¨º¡À¡À?¨¢?
//
//#ifdef _USE_XFT
//	p1 = messageDown[i].strMsg;
//
